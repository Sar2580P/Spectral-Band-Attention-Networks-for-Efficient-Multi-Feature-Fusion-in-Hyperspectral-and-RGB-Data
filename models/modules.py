...
class Composite(nn.Module):
  def __init__(self , in_channels, out_channels, kernel_size=(3,3),padding = 'same',
               stride = 1 , activation = 'prelu', apply_BN = True):
...
#   def __init__(self, in_channels,sparsity_threshold, r=2 ):
...
#       self.batch_norm_a = nn.BatchNorm1d(in_channels)
#       self.conv1d_a = nn.Conv1d(in_channels = 1 , out_channels = in_channels//r , kernel_size = 32)
#       self.batch_norm_b = nn.BatchNorm1d(in_channels)
...
#     vector = self.batch_norm_b(vector)  # (B,168)
#     # self.channel_weights = self.sigmoid(vector)  # (B,168)
#     self.channel_weights = vector # (B,168) , not using sigmoid, to avoid vanishing gradients
#     sparse_channel_weights = torch.where(self.channel_weights >= self.sparsity_threshold, torch.sigmoid(self.channel_weights), torch.zeros_like(vector))


#     # Multiply the image and vector along the channel dimension.
#     # vector: (C,)          x: (B, C, H, W)
#     output = x * sparse_channel_weights.unsqueeze(2).unsqueeze(3)  # (B, C, H, W)
#     return output
#___________________________________________________________________________________________________________________
